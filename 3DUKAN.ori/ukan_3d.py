...
class UKAN(nn.Module):
    ...
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        ...
        # self.patch_embed3 = PatchEmbed3D(img_size=(img_size[0]//4, img_size[1]//4, img_size[2]//4), patch_size=(3,3,3), stride=(1,1,1), in_chans=embed_dims[0], embed_dim=embed_dims[1])
        ...
        # self.patch_embed4 = PatchEmbed3D(img_size=(img_size[0]//4, img_size[1]//4, img_size[2]//4), patch_size=(3,3,3), stride=(1,1,1), in_chans=embed_dims[1], embed_dim=embed_dims[2])
    ...
    def forward(self, x):
        ...
        ### Stage 4
        # 修正点3: 元の構造に戻す(scale_factorを(2,2,2)に変更)
        out = F.relu(F.interpolate(self.decoder1(out), scale_factor=(2,2,2),mode ='trilinear'))
        # out = F.relu(F.interpolate(self.decoder1(out), scale_factor=(1,1,1),mode ='trilinear'))
        ...
        out = self.dnorm3(out)
        out = out.reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
        # 修正点4: 元の構造に戻す(scale_factorを(2,2,2)に変更)
        out = F.relu(F.interpolate(self.decoder2(out), scale_factor=(2,2,2),mode ='trilinear'))
        # out = F.relu(F.interpolate(self.decoder2(out), scale_factor=(1,1,1),mode ='trilinear'))
...
# with torch.no_grad():
#     out = model(x)
# print(out.shape)  # Expected output shape: (1, 1, 64, 256, 256)
