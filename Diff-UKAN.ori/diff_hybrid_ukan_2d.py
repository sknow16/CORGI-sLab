...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.ch_mult)):
            for _ in range(self.num_res_blocks):
                h = self.encoder.pop(0)(h, None)
            h_skip.append(h)
            if i != len(self.ch_mult) - 1:
                h = self.downblocks.pop(0)(h, None)
        
        return h_skip


class UKan_Hybrid(nn.Module):
    def __init__(self, T, ch, ch_mult, attn, img_size, num_res_blocks, dropout):
        super().__init__()
        assert all([i < len(ch_mult) for i in attn]), 'attn index h of bound'
        tdim = ch*4 # 256? or ch*4
        self.time_embedding = TimeEmbedding(T, ch, tdim)
        attn = []
        self.image_encoder = ImageEncoder(ch, ch_mult, num_res_blocks, dropout)
        self.head = nn.Conv2d(3, ch, kernel_size=3, stride=1, padding=1)
        ...
        for i, mult in enumerate(ch_mult):
            ...
            for j in range(num_res_blocks):
                if j == 0:
                    now_ch = now_ch+h_ch
                self.encoderblocks.append(ResBlock(
        ...
        self.patch_embed3 = OverlapPatchEmbed(img_size=img_size // 4, patch_size=3, stride=1, in_chans=embed_dims[0], embed_dim=embed_dims[1])
        self.patch_embed4 = OverlapPatchEmbed(img_size=img_size // 8, patch_size=3, stride=1, in_chans=embed_dims[1], embed_dim=embed_dims[2])
        ...
        self.decoder1 = D_SingleConv(embed_dims[2], embed_dims[1], tdim=tdim)  
        self.decoder2 = D_SingleConv(embed_dims[1], embed_dims[0], tdim=tdim)  
    ...
    def forward(self, x, t, y):
        ...
        for i in range(len(self.ch_mult)):
            ...
            for _ in range(self.num_res_blocks):
                h = self.encoderblocks.pop(0)(h, temb)
                hs.append(h)
            if i != len(self.ch_mult) - 1:
                h = self.downblocks.pop(0)(h, temb)
                hs.append(h)
...
if __name__ == '__main__':
    batch_size = 8
    model = UKan_Hybrid(
        T=1000, ch=128, ch_mult=[1, 2, 2, 2], attn=[], img_size=256,
