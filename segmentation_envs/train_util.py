...
def create_crop(x, crop_size):
    ...
    for i in range(n):
        ...
        x[i, :, left_points[i][0]:left_points[i][0]+crop_size, left_points[i][1]:left_points[i][1]+crop_size] = torch.clamp(x[i, :, left_points[i][0]:left_points[i][0]+crop_size, left_points[i][1]:left_points[i][1]+crop_size]+crop_color, 0, 1)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for image in tqdm(self.train_loader): # バーがでてくる
                x, cond = create_crop(image.clone(), crop_size=64)
                x = x.to(self.device)
