...
class ImageEncoder(nn.Module):
    ...
    # 順伝播を定義
    def forward(self, x):
        x1 = self.in_conv(x) # (32, 128, 128) 
        x = self.down(x1)  # (32, 128, 128) -> (32, 64, 64)
        x2 = self.conv1(x)  # (64, 64, 64)
        x = self.down(x2)  # (64, 64, 64) -> (64, 32, 32)
        x3 = self.conv2(x)  # (128, 32, 32)
        x = self.down(x3)  # (128, 32, 32) -> (128, 16, 16)
        x4 = self.conv3(x)  # (256, 16, 16)
        return x1, x2, x3, x4
...
class PatchSegUnet(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, cond_in_ch=3 ,hidden_ch=32, time_embed_dim=100):
        ...
        self.down1 = ResBlock(hidden_ch+hidden_ch*2+hidden_ch, hidden_ch*4, time_embed_dim)
        self.down2 = ResBlock(hidden_ch*4+hidden_ch*4+hidden_ch*2, hidden_ch*8, time_embed_dim)
        self.down3 = ResBlock(hidden_ch*8+hidden_ch*4+hidden_ch*4, hidden_ch*16, time_embed_dim)
...
# x = torch.randn(1,3,128,128) # maskは3ch
# t = torch.tensor([1])

# cond1 = torch.randn(1,64,128,128)
# cond2 = torch.randn(1,128,64,64)
# cond3 = torch.randn(1,128,32,32)
# cond4 = torch.randn(1,256,16,16)

# cond = [cond1, cond2, cond3, cond4]
# patch_imgage = torch.randn(1,1,128,128)
